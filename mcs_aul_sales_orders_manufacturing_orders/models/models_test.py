...
class InheritButtonMold(models.Model):
    ...
    total_ng_time_tracking = fields.Float(string='Sub Total NG', comppute="_compute_total_ng_time_tracking")
    ...
    def _compute_total_ng_time_tracking(self):
        for r in self:
            ...
            r.total_ng_time_tracking = ng_total
    ...
    def _compute_total_hasil_ok_produksi(self):
        for r in self:
            ...
            for line in r.validate_id:
                r.total_hasil_ok_produksi += line.ok_qty_prod
                total_ng_ng += line.ng_qty_prod
                r.total_hasil_ok += line.ok_qty
                r.total_hold_ok += line.hold_ok_qty
                r.total_hold_ng += line.ng_qty
            if r.total_ng_time_tracking == 0:
                r.total_ng = total_ng_ng
            elif r.total_ng_time_tracking >= 1:
                r.total_ng = total_ng_ng + r.total_ng_time_tracking
...
class MrpWorkcenterProductivity(models.Model):
    ...
    ng_ratio = fields.Float(string='NG Ratio', compute="_compute_ng_ratio")
    actual_cycle = fields.Float(string='Actual Cycle Time')
    
    @api.depends('ng_ratio')
